...
for t=1:400  %total simulation time
    t
...
%         hyp_robot(2)=hyp_robot(2);
    ...
    elseif (SWARM_FLAG==1)
%        break;
        %Two proceduires for defining the trajectory of the virtual robot
        
% %         %First method is to update the (x,y) coordinates of the virtual
% %         %robot
% %             hyp_robot(1)=hyp_robot(1)+0.01*t
% %             hyp_robot(2)=hyp_robot(2)+sin(2*pi*(1/100)*t)
...
%             fyv=10;
            ...
            [T,X_virtual]=ode45(fx_virtual,[0,0.1],[hyp_robot(1);xdot_virtual(i)]);
            [m,z] = size(X_virtual);
            ...
            [T,Y_virtual]=ode45(fy_virtual,[0,0.1],[hyp_robot(2);ydot_virtual(i)]);
            [m,z] = size(Y_virtual);
