...
% First Obstacle
origin_of_rectangle(1,:)=[20,20];
width(1)=4;
height(1)=3;

% Second Obstacle
origin_of_rectangle(2,:)=[40,30];
width(2)=4;
height(2)=3;

figure(1);
rectangle('Position',[origin_of_rectangle(1,:) width(1) height(1)],'Curvature',[0.05 0.05]),...
    axis equal,hold on
rectangle('Position',[origin_of_rectangle(2,:) width(2) height(2)],'Curvature',[0.05 0.05]),...
    axis equal,hold on
...
for i=1:number_of_obstacles
    ...
    xa(i)=origin_of_rectangle(i,1);
    ya(i)=origin_of_rectangle(i,2);
    ...
    y1(i,:)=real(sqrt((1-A(i)^2*(x-x0(i)).^2)/B(i)^2)+y0(i));
    y2(i,:)=real(-sqrt((1-A(i)^2*(x-x0(i)).^2)/B(i)^2)+y0(i));
    
    plot(x,y1(i,:),'g',x,y2(i,:),'g'),axis tight,grid on,hold on
    
    v1_expansion(i,:)=v1(i):v1(i)/10:v1(i)*1.25;
    
    A1(i,:)=sqrt(1./(2*(v1_expansion(i,:).^2)));
    B1(i,:)=(B(i)/A(i))*A1(i,:);
    
    for j=1:length(B1(1,:))
        
        y1(i,:)=real(sqrt((1-A1(i,j)^2*(x-x0(i)).^2)/B1(i,j)^2)+y0(i));
        y2(i,:)=real(-sqrt((1-A1(i,j)^2*(x-x0(i)).^2)/B1(i,j)^2)+y0(i));
        plot(x,y1(i,:),'--k',x,y2(i,:),'--k'),axis tight,grid on,hold on
        %     end
    end
